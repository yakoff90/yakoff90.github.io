    ...
    function InQualityService() {
        ...
        self.list = function(params, onComplete, onError) {
            ...
            network.silent(JSON_URL, async function(json) {
                ...
                try {
                    var items = await Promise.all(json.results.map(async function(item) {
                        var id = item.id || item.title || Math.random();
                        if (cache[id]) return cache[id];

                        let title = item.title || '';
                        let poster = item.poster_path || '';
                        let translated = await fetchTMDB(title);
                        let card = {
                            id: id,
                            title: translated.title || title,
                            original_title: translated.title || title,
                            poster_path: translated.poster_path || poster,
                            vote_average: translated.vote_average || item.vote_average || 0,
                            release_date: translated.release_date || item.release_date || '',
                            quality: item.release_quality || '',
                            original_language: translated.original_language || item.original_language || '',
                            source: 'tmdb',
                            type: 'movie'
                        };
                        cache[id] = card;
        ...
        async function fetchTMDB(title) {
            return new Promise(resolve => {
                ...
                }, function() { resolve({}); }, { language: 'uk-UA', api_key: TMDB_API_KEY, type: 'movie' });
            });
        }

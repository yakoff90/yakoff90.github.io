(function() {
    ...
    var CONFIG = {
        omdbApiKey: '4ef0d7355d9ffb5151e987764708ce96', // OMDb ключ
        tmdbApiKey: '4ef0d7355d9ffb5151e987764708ce96', // TMDB ключ (той самий можна)
        enabled: true
    };
    ...
    // --- Запит до TMDB, щоб отримати imdb_id ---
    function fetchImdbId(tmdbId) {
        return new Promise(function(resolve, reject) {
            var url = "https://api.themoviedb.org/3/tv/" + tmdbId + "/external_ids?api_key=" + CONFIG.tmdbApiKey;
            ...
            xhr.onerror = function () { reject(new Error("Помилка мережі TMDB")); };
            xhr.send();
        });
    }
    ...
    function fetchRating(imdbId) {
        return new Promise(function(resolve, reject) {
            if (!imdbId) return reject(new Error("IMDb ID відсутній"));
            var url = "https://www.omdbapi.com/?i=" + imdbId + "&apikey=" + CONFIG.omdbApiKey;
            var xhr = new XMLHttpRequest();
            xhr.open("GET", url, true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
            ...
            xhr.onerror = function () { reject(new Error("Помилка мережі OMDb")); };
            xhr.send();
        });
    }
    ...
    function addRatingBadge(cardEl) {
        ...
        // 1. Отримуємо imdb_id через TMDB API
        fetchImdbId(data.id)
            .then(function(imdbId) {
                if (!imdbId) throw new Error("imdb_id не знайдено");
                return fetchRating(imdbId);
            })

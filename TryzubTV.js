(function () {
  ...
  // Адаптированные функции для Tizen
  ...
  // Адаптированная функция запроса для Tizen
  function request$2(url) {
    return new Promise(function (resolve, reject) {
      if (IS_TIZEN) {
        // Для Tizen используем более простой подход
        try {
  ...
  // Адаптированный модуль Player для Tizen
  ...
  // Адаптированный модуль QR для Tizen
  ...
  function openQrModal() {
    ...
    // Упрощенный QR код для Tizen
    if (IS_TIZEN && Lampa.Utils && Lampa.Utils.qrcode) {
      try {
        Lampa.Utils.qrcode(QR_URL, qrElement, function (error) {
          if (error) {
            console.error('QR code generation failed:', error);
            qrElement.html('<div style="color: white; padding: 20px;">QR код не доступен на этом устройстве</div>');
          }
        });
      } catch (e) {
        qrElement.html('<div style="color: white; padding: 20px;">QR код не доступен на этом устройстве</div>');
      }
    } else if (Lampa.Utils && Lampa.Utils.qrcode) {
      Lampa.Utils.qrcode(QR_URL, qrElement, function (error) {
        console.error(error);
      });
    } else {
  ...
  // Адаптированный SocketManager для Tizen
  var SocketManager = function () {
    ...
    function createInstance() {
      var connect = function connect() {
        return new Promise(function (resolve, reject) {
          // Для Tizen используем альтернативный подход без WebSocket
          if (IS_TIZEN) {
            console.log('TryzubTV: Tizen detected, using alternative connection method');
            // Генерируем локальный ключ для Tizen
          ...
          try {
            ...
            reject('TryzubTV: WebSocket not supported');
            return;
          }
          ...
          socket.onclose = function () {
            ...
            if (reconnectAttempts < maxReconnectAttempts) {
              reconnectAttempts++;
              console.log('TryzubTV: Attempting reconnect', reconnectAttempts);
              setTimeout(function() {
      ...
      var disconnect = function disconnect() {
        if (socket) {
          try {
            ...
            console.error('TryzubTV: Error closing socket', e);
          }
        }
        socket = null;
        key = null;
  ...
  // Основной компонент с адаптациями для Tizen
  function appendReplayOverlay(cardItem, cardData) {
    try {
      if (IS_TIZEN) {
        // Упрощенная версия для Tizen
    ...
    _playReplay = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(cardData) {
      ...
      return _regenerator().w(function (_context) {
        while (1) switch (_context.n) {
          ...
          case 3:
            key = _context.v;
            // Для Tizen используем прямой URL без ключа
            if (IS_TIZEN) {
              newLink = "https://vod-maincast.cosmonova-broadcast.tv/".concat(cardData.salo_vod_id, "/master.m3u8");
            } else {
  ...
  // API для replay с адаптацией для Tizen
  ...
  // Компонент для replay с адаптацией для Tizen
                  ...
                  return _regenerator().w(function (_context) {
                    while (1) switch (_context.n) {
                      ...
                      case 3:
                        ...
                        // Для Tizen используем прямой URL без ключа
                        if (IS_TIZEN) {
                          newLink = "https://vod-maincast.cosmonova-broadcast.tv/".concat(card_data.salo_vod_id, "/master.m3u8");
                        } else {
  ...
  function startPlugin() {
    ...
    // Адаптированные стили для Tizen
    ...
    if (window.appready) {
      setTimeout(addMenu, 1000); // Задержка для Tizen
    } else {
  ...
  if (Lampa.Manifest.app_digital >= 300) {
    // Задержка для инициализации на Tizen
    if (IS_TIZEN) {

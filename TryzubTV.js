(function () {
  ...
  console.log('TryzubTV: Платформа - Tizen:', IS_TIZEN);

  // Базові константи
  var API_BASE = 'https://dyvy.tv/api/v1';
  var REPLAY_BASE$1 = 'https://a.maincast.tv/items';
  var REPLAY_LIMIT = 10;
  ...
  // Функція запиту з CORS проксі для Tizen
  function request$2(url) {
    return new Promise(function (resolve, reject) {
      console.log('TryzubTV: Запит до:', url);
      
      // Для Tizen використовуємо проксі
      var requestUrl = IS_TIZEN ? 
        'https://api.allorigins.win/raw?url=' + encodeURIComponent(url) :
        url;
      
      if (IS_TIZEN) {
        console.log('TryzubTV: Використовуємо проксі для Tizen');
      }
      
      // Спробуємо різні методи
      if (Lampa && Lampa.Network && Lampa.Network.silent && !IS_TIZEN) {
        // Оригінальний метод для Lampa
        Lampa.Network.silent(requestUrl, resolve, reject);
      } else {
        // XMLHttpRequest для всіх платформ
        try {
            ...
            if (xhr.readyState === 4) {
              if (xhr.status === 200 || xhr.status === 0) {
                try {
                  ...
                  console.error('TryzubTV: Помилка парсингу JSON:', e);
                  reject(e);
                }
              } else {
                console.error('TryzubTV: Помилка HTTP:', xhr.status);
          ...
          xhr.onerror = function () {
            console.error('TryzubTV: Помилка мережі');
            reject(new Error('Помилка мережі'));
          };
          
          xhr.ontimeout = function () {
            console.error('TryzubTV: Таймаут запиту');
            reject(new Error('Таймаут'));
          };
          
          xhr.send();
        } catch (error) {
          console.error('TryzubTV: Виняток при запиті:', error);
          reject(error);
        }
      }
    });
  ...
  function mapReplayToCard(vod) {
    try {
      ...
      console.error('TryzubTV: Помилка перетворення replay:', e);
      return null;
    }
  }
  ...
  function _fetchCategories() {
    _fetchCategories = _asyncToGenerator(function* () {
      try {
        var response = yield request$2("".concat(API_BASE, "/categories?is_main=1"));
        return response && response.data ? response.data : [];
      } catch (error) {
        console.error('TryzubTV: Помилка завантаження категорій:', error);
        return [];
      }
    });
    return _fetchCategories.apply(this, arguments);
  ...
  function _fetchReplayDisciplines() {
    _fetchReplayDisciplines = _asyncToGenerator(function* () {
      try {
        // Використовуємо проксі для Tizen
        var url = IS_TIZEN ? 
          'https://api.allorigins.win/raw?url=' + encodeURIComponent("".concat(REPLAY_BASE$1, "/discipline?filter={\"category\":\"sport\"}&fields=name,id,icon")) :
          "".concat(REPLAY_BASE$1, "/discipline?filter={\"category\":\"sport\"}&fields=name,id,icon");
        
        var response = yield request$2(url);
        return response && response.data ? response.data : [];
      } catch (error) {
        console.error('TryzubTV: Помилка завантаження дисциплін:', error);
        return [];
      }
    });
    return _fetchReplayDisciplines.apply(this, arguments);
  ...
  function _fetchCatalogLines() {
    _fetchCatalogLines = _asyncToGenerator(function* () {
      try {
        ...
        return tvLines.concat(replayLines);
      } catch (error) {
        console.error('TryzubTV: Помилка завантаження каталогу:', error);
        return [];
      }
    });
    return _fetchCatalogLines.apply(this, arguments);
  ...
  function buildLineShell(meta, LineModule, index) {
    ...
    if (meta.source === SOURCE_TV) {
      ...
      var morePrefix = Lampa && Lampa.Lang ? Lampa.Lang.translate('tryzubtv_more_videos') : 'Усі відео:';
      params.more = {
  ...
  function _loadMain() {
    _loadMain = _asyncToGenerator(function* () {
      try {
        ...
        console.error('TryzubTV: Помилка в loadMain:', error);
  ...
  function _loadCategory$1() {
    _loadCategory$1 = _asyncToGenerator(function* (categorySlug, categoryTitle, oncomplete, onerror) {
      try {
        var url = "".concat(API_BASE, "/channels?category_slug=").concat(encodeURIComponent(categorySlug), "&limit=500");
        var response = yield request$2(url);
        ...
        var lineTitle = total ? "".concat(title, " \xB7 ").concat(total) : title;
        
        var items = channels.filter(isFreeChannel).filter(function (channel) {
        ...
        console.error('TryzubTV: Помилка завантаження категорії:', error);
        if (onerror) onerror(error);
        oncomplete([{
    ...
    _loadLineItems = _asyncToGenerator(function* (lineData) {
      try {
        ...
        if (source === SOURCE_TV) {
          ...
          var url = "".concat(API_BASE, "/channels?category_slug=").concat(encodeURIComponent(slug), "&limit=500");
          var response = yield request$2(url);
          var channels = response && response.data ? response.data : [];
          var total = response && response.meta && response.meta.total ? response.meta.total : channels.length;
          ...
          // Використовуємо проксі для Tizen
          var replayUrl = "".concat(REPLAY_BASE$1, "/vod?filter={\"discipline\":").concat(disciplineId, "}&sort=-date&limit=").concat(REPLAY_LIMIT + 1);
          var requestUrl = IS_TIZEN ? 
            'https://api.allorigins.win/raw?url=' + encodeURIComponent(replayUrl) :
            replayUrl;
          
          var _response = yield request$2(requestUrl);
        ...
        console.error('TryzubTV: Помилка завантаження елементів лінії:', error);
        return { items: [], total: 0, total_pages: 1 };
      }
    });
    return _loadLineItems.apply(this, arguments);
  ...
  function buildChannelEpgUrl(slug) {
    return "".concat(API_BASE, "/channels/").concat(encodeURIComponent(slug), "?expand=epg_items");
  }
  ...
  function renderProgramList(container, items, position) {
    ...
    if (!items.length) {
      ...
      empty.innerHTML = "<span>".concat(Lampa && Lampa.Lang ? Lampa.Lang.translate('tryzubtv_now_empty') : 'Немає даних EPG', "</span>");
  ...
  function _fetchCategoryChannels() {
    _fetchCategoryChannels = _asyncToGenerator(function* (categorySlug) {
      try {
        ...
        console.error('TryzubTV: Помилка завантаження каналів категорії:', error);
        return [];
      }
    });
    return _fetchCategoryChannels.apply(this, arguments);
  ...
  function playIptvList(channels, startIndex, groupTitle) {
    ...
    var data = {
      ...
      onGetChannel: function onGetChannel(position) {
        ...
        fetchChannelEpg(selected.slug).then(function (data) {
          ...
          console.error('TryzubTV: Помилка завантаження EPG каналу', error);
          epgCache[selected.slug] = {
            items: [],
            position: 0
          };
      ...
      onPlaylistProgram: function onPlaylistProgram(selected, position) {
        ...
        fetchChannelEpg(selected.slug).then(function (data) {
          ...
          console.error('TryzubTV: Помилка завантаження плейліста', error);
    ...
    if (Lampa && Lampa.Player && Lampa.Player.iptv) {
      ...
      console.error('TryzubTV: Lampa.Player.iptv недоступний');
  ...
  function playChannel(cardData) {
    ...
    fetchChannelEpg(cardData.tryzubtv_slug).then(function (data) {
      ...
      console.error('TryzubTV: Помилка завантаження списку EPG', error);
    });
  }
  ...
  function openQrModal() {
    var html = $('<div class="tryzubtv-qr-modal" style="display:flex;flex-direction:column;gap:1.2em;align-items:center;text-align:center;">' + 
      ...
      "<div class=\"account-modal-split__text\"><img src=\"".concat(QR_CARD_POSTER, "\" class=\"tryzubtv-qr-modal__img\"><br />").concat(QR_BODY, "</div>") + 
      '</div>' + 
    ...
    if (Lampa.Utils && Lampa.Utils.qrcode) {
      ...
      qrElement.html('<div style="color: white; padding: 20px;">QR код недоступний на цьому пристрої</div>');
    }
  ...
  var SocketManager = function () {
    ...
    function createInstance() {
      var connect = function connect() {
        return new Promise(function (resolve, reject) {
          if (IS_TIZEN) {
            console.log('TryzubTV: Tizen виявлено, використовуємо альтернативний метод');
            key = 'tizen_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
            resolve(key);
            return;
          }
          ...
          try {
            ...
            reject('TryzubTV: WebSocket не підтримується');
            return;
          }
          
          socket.onopen = function () {
            console.log('TryzubTV: WebSocket підключено');
          };
          ...
          socket.onerror = function () {
            console.error('TryzubTV: Помилка WebSocket');
            reject('TryzubTV: Помилка WebSocket');
            disconnect();
          };
          
          socket.onclose = function () {
            console.log('TryzubTV: WebSocket відключено');
            socket = null;
            key = null;
          };
        });
      ...
      var handleMessage = function handleMessage(event, resolve, reject) {
        try {
          ...
          if (data.type === 'auth' && data.payload && data.payload.hash) {
            ...
            console.log('TryzubTV: Ключ отримано');
            resolve(key);
          } else {
            reject('TryzubTV: Невірна відповідь авторизації');
          }
        } catch (e) {
          reject('TryzubTV: Помилка парсингу повідомлення');
        }
      };
      ...
      var listen = function listen() {
        Lampa.Player.listener.follow('destroy', function () {
          console.log('TryzubTV: Плеєр знищено, закриваємо сокет.');
          disconnect();
        });
      };
  ...
  function appendReplayOverlay(cardItem, cardData) {
    try {
      ...
      console.error('TryzubTV: Помилка overlay replay картки', e);
    }
  }
  ...
  function _playReplay() {
    _playReplay = _asyncToGenerator(function* (cardData) {
      ...
      try {
        ...
        var key = yield socket.connect();
        var newLink = "https://vod-maincast.cosmonova-broadcast.tv/".concat(cardData.salo_vod_id, "/master.m3u8?key=").concat(key);
        Lampa.Player.play({
          title: cardData.title,
          url: newLink
        });
      } catch (error) {
        console.error('TryzubTV: Помилка відтворення replay', error);
  ...
  function loadLineContent(lineItem, lineData) {
    ...
    Api$1.loadLineItems(lineData).then(function (payload) {
      ...
      console.error('TryzubTV: Помилка завантаження лінії', error);
  ...
  // Редактор каталогу (НАЛАШТУВАННЯ КАТЕГОРІЙ)
  ...
  function openCatalogEditor() {
    ...
    Api$1.fetchCatalogLines().then(function (lines) {
      ...
      console.error('TryzubTV: Помилка завантаження редактора каталогу', error);
  ...
  function initSettings() {
    ...
    // КНОПКА НАЛАШТУВАНЬ КАТАЛОГУ (повернуто!)
    SettingsApi.addParam({
      component: 'tryzubtv',
      param: {
  ...
  // API для replay
  var REPLAY_BASE = 'https://a.maincast.tv/items';
  ...
  function _loadCategory() {
    _loadCategory = _asyncToGenerator(function* (disciplineId, oncomplete, onerror) {
      try {
        // Використовуємо проксі для Tizen
        var url = "".concat(REPLAY_BASE, "/vod?filter={\"discipline\":").concat(disciplineId, "}&sort=-date&limit=100");
        var requestUrl = IS_TIZEN ? 
          'https://api.allorigins.win/raw?url=' + encodeURIComponent(url) :
          url;
        
        var response = yield request(requestUrl);
        ...
        console.error("TryzubTV: Не вдалося завантажити категорію replay ".concat(disciplineId), error);
        if (onerror) onerror(error);
      }
    });
    return _loadCategory.apply(this, arguments);
            ...
            card_item.use({
              onCreate: function onCreate() {
                try {
                  ...
                  console.error('TryzubTV: Помилка створення overlay replay картки', e);
                }
              },
              onEnter: function () {
                var _onEnter = _asyncToGenerator(function* () {
                  ...
                  try {
                    var socket = SocketManager.getInstance();
                    var key = yield socket.connect();
                    var newLink = "https://vod-maincast.cosmonova-broadcast.tv/".concat(card_data.salo_vod_id, "/master.m3u8?key=").concat(key);
                    Lampa.Player.play({
                      title: card_data.title,
                      url: newLink
                    });
                  } catch (error) {
                    console.error('TryzubTV: Помилка запуску відтворення replay', error);
                    Lampa.Noty.show('Не вдалося отримати ключ для відтворення.');
                  } finally {
                    Lampa.Loading.stop();
                  }
  ...
  function startPlugin() {
    ...
    var manifest = {
      ...
      version: '1.5.2',
      name: 'TryzubTV',
      description: 'Українські ТВ канали',
    ...
    if (window.appready) {
      setTimeout(addMenu, IS_TIZEN ? 2000 : 0);
    } else {

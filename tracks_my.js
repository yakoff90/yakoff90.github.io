...
(function () {
    ...
    function subscribeTracks(data) {
      ...
      function buildPanelTracksFromParse() {
        // Build track objects for Lampa.PlayerPanel from parsed ffprobe metadata
        ...
        // Filter out unknowns like dts/truehd when counts mismatch (legacy behavior)
      ...
      function buildPanelSubsFromParse() {
        if (!inited_parse) return [];
        var new_subs = [];
      ...
      function setTracks() {
        if (inited_parse) {
          var new_tracks = buildPanelTracksFromParse();
          if (new_tracks.length) {
            // Always push to PlayerPanel so UI shows labels regardless of platform
      ...
      function setSubs() {
        if (inited_parse) {
          var new_subs = buildPanelSubsFromParse();
          if (new_subs.length) {
      ...
      function canPlay() {
        ...
        // Prefer panel-based updates to ensure UI consistency across platforms
        setTracks();
        setSubs();
        Lampa.PlayerVideo.listener.remove('canplay', canPlay);
      }

      // Keep webos_* listeners but fall back to panel updates
      function setWebosTracks(video_tracks) {
        if (inited_parse) {
          // Try to write into video_tracks (may be read-only on some TVs)
          var parse_tracks = inited_parse.streams.filter(function (a) {
            return a.codec_type == 'audio';
          });
          ...
          parse_tracks.forEach(function (track, i) {
            if (video_tracks[i]) {
              try {
                ...
                // Some platforms have read-only properties — ignore
              }
            }
          });

          // ALSO update panel directly so labels are shown in Lampa UI
          var panelTracks = buildPanelTracksFromParse();
          if (panelTracks.length) Lampa.PlayerPanel.setTracks(panelTracks);
        }
      }
      ...
      function setWebosSubs(video_subs) {
        if (inited_parse) {
          ...
          // ALSO update panel directly
          var panelSubs = buildPanelSubsFromParse();
          if (panelSubs.length) Lampa.PlayerPanel.setSubs(panelSubs);
        }
      }
      ...
      function listenWebosSubs(_data) {
        ...
        // Update panel immediately if already parsed
      ...
      function listenStart() {
        ...
        reguest(data, function (result) {
          ...
          if (inited) {
            // Always update panel-based UI to ensure labels appear on all platforms
            setTracks();
            setSubs();

            // If we already have webos data, keep them in sync

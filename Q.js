 ...
 * Залишено лише якість та рейтинг на постері.
 * Якість - лівий нижній кут постера, рейтинг - правий нижній кут постера.
 */
(function () {
    'use strict';
    ...
    function initMarksJacRed() {
        ...
        function addMarksToContainer(element, movie) {
            ...
            // Отримуємо контейнер постера (картинку)
            var posterContainer = element.find('.card__poster, .poster, .card__view');
            if (!posterContainer.length) posterContainer = element;
            
            // Контейнер для мітки якості (лівий нижній кут постера)
            var qualityContainer = $('<div class="card-marks-quality"></div>');
            posterContainer.append(qualityContainer);
            
            // Контейнер для рейтингу (правий нижній кут постера)
            var ratingContainer = $('<div class="card-marks-rating"></div>');
            posterContainer.append(ratingContainer);
        ...
        function renderBadges(qualityContainer, ratingContainer, data, movie) {
            ...
            // Мітка якості (лівий нижній кут постера)
            if (data.resolution && data.resolution !== 'SD') {
            ...
            // Рейтинг (правий нижній кут постера)
        ...
        style.innerHTML = `
            ...
            .card__poster, .poster, .card__view {
                position: relative;
                overflow: hidden;
            }
            ...
            .card__mark--rating .mark-star { margin-right: 2px; font-size: 10px; }

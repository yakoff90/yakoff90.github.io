(function () {
    ...
    waitForLampa(function() {
        ...
        // Безпечне сховище з обмеженням для Tizen
        ...
        try { 
            var cachedSeasons = safeStorage.getItem('seasonBadgeCacheV5');
            if (cachedSeasons) seasonsCache = JSON.parse(cachedSeasons);
        } catch (e) {}
        ...
        function Cache(config) {
            ...
            self.set = function (id, value) {
                cleanupExpired();
                // Обмеження кількості записів для Tizen
                var keys = Object.keys(storage);
                if (keys.length >= config.size) {
                    var oldestKey = keys.reduce(function(a, b) { 
                        return storage[a].timestamp < storage[b].timestamp ? a : b; 
                    });
                ...
                inflight[meta.cacheKey] = new Promise(function (resolve) {
                    requestQueue.add(function () {
                        ...
                        return new Promise(function (res) { 
                            if (Lampa.Network && Lampa.Network.silent) {
                                Lampa.Network.silent(url, function (r) { res(isSuccessResponse(r)); }, function () { res(false); }, null, { timeout: CONFIG.timeout });
                            } else {
                                safeFetch(url).then(function(r) { return r.json(); }).then(function(r) { res(isSuccessResponse(r)); }).catch(function() { res(false); });
                            }
                        })
                        .then(function (isSuccess) { 
                            if (lmeCache) lmeCache.set(meta.cacheKey, isSuccess); 
                            resolve(isSuccess); 
                        })
        ...
        function fetchLogo(movie, itemElement) {
            ...
            function applyTextLogo() {
                ...
                if (view && view.length) view.append(textLogo);
            }
            ...
            function applyLogo(url) {
                if (url && url !== 'none') {
                    ...
                    img.onload = function() { 
                        var view = itemElement.find('.card__view');
                        if (view && view.length) view.append(img);
                    };
            ...
            fetch(PROXIES[0] + endpoint).then(r => r.json()).then(function(res) {
                ...
                if (res.logos && res.logos.length > 0) {
                    ...
                    if (found && Lampa.TMDB && Lampa.TMDB.image) {
                ...
                params: {
                    ...
                    emit: {
                        onCreate: function () {
                            ...
                            var iconHtml = iconUrl ? '<img src="' + iconUrl + '" class="title-btn-icon" style="width:24px;height:24px;margin-right:8px;vertical-align:middle;" onerror="this.style.display=\'none\'" />' : '';
                            item.append('<div class="title-btn-text" style="display:flex;align-items:center;justify-content:center;padding:12px;">' + iconHtml + '<span>' + title + '</span></div>');
                        },
                        onlyEnter: function () {
                            if (url) {
                                Lampa.Activity.push({
                ...
                params: {
                    ...
                    emit: {
                        onCreate: function () {
                            ...
                            item.empty(); 
                            item.append('<div class="collection-title" style="display:flex;align-items:center;justify-content:center;padding:12px;text-align:center;">' + collection.title + '</div>');
                        },
                        onlyEnter: function () {
                            Lampa.Activity.push({
                                url: collection.url,
                ...
                params: {
                    ...
                    emit: {
                        onCreate: function () {
                            ...
                            view.append('<div class="card-backdrop-overlay" style="background: rgba(0,0,0,0.65);position:absolute;top:0;left:0;right:0;bottom:0;"></div>');
                ...
                params: {
                    ...
                    emit: {
                        onCreate: function () {
                            ...
                            var imgUrl = imgUrlPath && Lampa.TMDB ? (PROXIES[0] + Lampa.TMDB.image('t/p/' + quality + imgUrlPath)) : '';
                            ...
                            view.append('<div class="card-backdrop-overlay" style="background: rgba(0,0,0,0.5);position:absolute;top:0;left:0;right:0;bottom:0;"></div>');
                ...
                params: {
                    ...
                    emit: {
                        onCreate: function () {
                            ...
                            var quality = Lampa.Storage.get('ym_img_quality', 'w300');
                            var imgUrl = movie.backdrop_path && Lampa.TMDB ? PROXIES[0] + Lampa.TMDB.image('t/p/' + quality + movie.backdrop_path) : '';
                            if (imgUrl) {
                                view.css({
                            ...
                            view.append('<div class="card-backdrop-overlay" style="background: rgba(0,0,0,0.4);position:absolute;top:0;left:0;right:0;bottom:0;"></div>');
                            ...
                            item.append('<div class="custom-title-bottom" style="position:absolute;bottom:40px;left:10px;right:10px;color:#fff;font-size:14px;font-weight:bold;text-shadow:0 1px 2px rgba(0,0,0,0.8);z-index:2;">' + (movie.title || movie.name) + '</div>');
                            item.append('<div class="custom-overview-bottom" style="position:absolute;bottom:10px;left:10px;right:10px;color:#ddd;font-size:11px;text-shadow:0 1px 1px rgba(0,0,0,0.8);z-index:2;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">' + descText + '</div>');
                        },
                        onlyEnter: function () {
                            var mType = movie.media_type || (movie.name ? 'tv' : 'movie');
                            Lampa.Activity.push({ url: '', component: 'full', id: movie.id, method: mType, card: movie, source: movie.source || 'tmdb' });
        ...
        function loadHistoryRow(callback) {
            ...
            try {
                ...
                if (validFavs.length > 0) {
                    ...
                    randFavImg = imgUrlPath && Lampa.TMDB ? (PROXIES[0] + Lampa.TMDB.image('t/p/' + quality + imgUrlPath)) : '';
                }
            } catch(e) {}
            ...
            try {
                ...
                if (validHist.length > 0) {
                    ...
                    let quality = Lampa.Storage.get('ym_img_quality', 'w300');
                    let imgUrlPath = randItem.backdrop_path || randItem.poster_path;
                    randHistImg = imgUrlPath && Lampa.TMDB ? (PROXIES[0] + Lampa.TMDB.image('t/p/' + quality + imgUrlPath)) : '';
                }
            } catch(e) {}
        ...
        function start() {
            ...
            Lampa.Media.on('create', function (component) {
                if (component.name === 'card' && Lampa.Storage.get('uas_show_flag', true)) {
                    var cardHtml = component.html;
                    var mediaMeta = createMediaMeta(component.movie);
                    if (mediaMeta) {
                        loadFlag(mediaMeta).then(function (hasUa) {
                            if (hasUa && component.html && component.html.querySelector) {
                                renderFlag(component.html);
                            }
                        });
                    }
                }
            ...
            Lampa.Media.on('create', function (component) {
                if (component.name === 'card' && component.movie && component.movie.media_type === 'tv') {
                    var cardHtml = component.html;
                    var tvId = component.movie.id;
                    if (tvId && Lampa.Storage.get('uas_show_flag', true)) {
                        fetchSeriesData(tvId).then(function (tvData) {
                            if (tvData && component.html) renderSeasonBadge(component.html, tvData);
                        }).catch(function() {});
                    }
                }
            });

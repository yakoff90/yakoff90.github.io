(function () {
		...
		var state = {
			...
			destroy: function () {
				...
				if (container) {
					...
					container.classList.remove("tv-optimized");
				}
	...
	function handleLineAppend(items, line, data) {
		...
		if (line.params) {
			line.params.items_per_row = isSamsungTV ? 6 : 12;
			if (line.params.items) line.params.items.view = isSamsungTV ? 6 : 12;
		}
	...
	function getWideStyles() {
		return `<style>
					...
					.tv-optimized .new-interface-info__body {
						width: 85%;
						padding-top: 1.5em;
					}
					...
					.tv-optimized .new-interface .full-start__rate {
						font-size: 1.5em;
					}
	...
	function initializeSettings() {
		...
		function setDefaultSettings() {
			...
			Lampa.Storage.set("background_resolution", isSamsungTV ? "w1280" : "original");
			...
			Lampa.Storage.set("interface_size", "small");
			Lampa.Storage.set("tv_optimization", isSamsungTV ? "true" : "false");
		}
	}
	...
	if (isSamsungTV) {
		...
		$(window).on('keydown', function(e) {
			...
			if (e.keyCode === 38 || e.keyCode === 40) { // Up/Down
				...
				if (active.length) {
					setTimeout(function() {
						var data = findCardData(active[0]);
						if (data) {
							var state = active.closest('.new-interface').data('state');

(function() {
  ...
  var hostkey = 'http://kinoxa.click'.replace('http://', '').replace('https://', '');
  ...
  if (!window.rch_nws || !window.rch_nws[hostkey]) {
    ...
    window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
      if (window.rch_nws[hostkey].type == undefined) {
        ...
        if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
          ...
          net.silent('http://kinoxa.click'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function () {
            check(true);
          }, function () {
            check(false);
          }, false, {
    ...
    window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
      window.rch_nws[hostkey].typeInvoke('http://kinoxa.click', function () {
              ...
              new Response(compressedStream).arrayBuffer()
                .then(function (compressedBuffer) {
                  ...
                  if (compressedArray.length > html.length) {
                    ...
                    $.ajax({
                      url: 'http://kinoxa.click/rch/gzresult?id=' + rchId,
                      type: 'POST',
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://kinoxa.click/js/nws-client-es5.js"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account('http://kinoxa.click/lite/withsearch'), function(json) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('http://kinoxa.click/player-inner/' + element.url);
      return;
    }

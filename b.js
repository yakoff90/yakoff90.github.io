(function () {
    ...
    const JACRED_URL = Lampa.Storage.get('jac.red') || 'jac.red';
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addComponent({
            component: 'uabadges',
            name: 'UA Бейджі',
            description: 'Рік, рейтинг, сезон, якість, прапорець',
        ...
        Lampa.SettingsApi.addParam({
            component: 'uabadges',
            param: { name: 'jac_red_url', type: 'input', default: 'jac.red' },
            field: { name: 'JacRed URL', description: 'Адреса JacRed для отримання якості' }
        });
    }
    ...
    function start() {
        ...
        // Оновлюємо JACRED_URL з налаштувань
        var savedUrl = Lampa.Storage.get('jac_red_url');
        if (savedUrl) {
            window.JACRED_URL = savedUrl;
        }
        ...
        Lampa.Listener.follow('settings', function(e) {
            if (e.type === 'change') {
                if (e.name === 'jac_red_url') {
                    window.JACRED_URL = Lampa.Storage.get('jac_red_url');
                }
                if (e.name && e.name.startsWith('show_')) {
                    updateVisibility();

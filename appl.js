(function () {
    ...
    function initializePlugin() {
        console.log('Applecation', 'v' + APPLECATION_VERSION + ' для Samsung TV');
    ...
    /**
     * Инициализация управления оверлеем
     */
    function initializeOverlayControls() {
        // Глобальная переменная для отслеживания открытого оверлея
        window.applecationActiveOverlay = null;
        
        // Обработчик кнопки "Назад" для закрытия оверлея
        $(document).off('keydown.applecation').on('keydown.applecation', function(e) {
            // Код кнопки "Назад" на Samsung TV (27 = ESC, 10009 = Back)
            if ((e.keyCode === 27 || e.keyCode === 10009) && window.applecationActiveOverlay) {
                e.preventDefault();
                e.stopPropagation();
    ...
    const translations = {
        ...
        text_scale_desc: {
            ...
            bg: 'Мащаב на текста с данни за филма',
    ...
    // Применяем масштабирование контента - ФИКСИРОВАНО ДЛЯ SAMSUNG TV
    function applyScales() {
        ...
        // Рассчитываем значения с базовыми размерами
        const baseLogoWidth = 35; // базовый max-width в vw
        const baseLogoHeight = 180; // базовый max-height в px
        const baseTextWidth = 35; // базовый max-width в vw для описания

        // Адаптивные значения для Samsung TV
        const logoWidth = Math.min(baseLogoWidth * logoScale / 100, 50); // Ограничиваем максимальную ширину
        const logoHeight = baseLogoHeight * logoScale / 100;
        const textWidth = Math.min(baseTextWidth * textScale / 100, 50); // Ограничиваем максимальную ширину текста
        ...
        const scaleStyles = `
            <style data-id="applecation_scales">
                ...
                /* Масштаб для оверлея с описанием */
        ...
        const overlayTemplate = `
            <div class="applecation-description-overlay" style="display: none;">
                ...
                <div class="applecation-description-overlay__content selector">
                    <div class="applecation-description-overlay__close selector" tabindex="0">
            ...
            <div class="full-start-new__right">
                <div class="applecation__left">
                    ...
                    <div class="applecation__content-wrapper">
                        ...
                        <div class="applecation__description-wrapper selector">
    ...
    function addStyles() {
        const styles = `<style>
/* Основной контейнер - оптимизирован для Samsung TV */
.applecation {
    transition: all .3s;
}
...
/* Логотип - оптимизация для Samsung TV */
.applecation__logo {
    margin-bottom: 0.5em;
    opacity: 0;
...
/* Оверлей для полного описания - ОПТИМИЗИРОВАН ДЛЯ SAMSUNG TV */
...
/* Улучшения для пульта Samsung TV */
.selector {
...
/* Улучшение плавности анимаций для Samsung TV */
.applecation * {
...
@media (min-width: 1920px) {
    ...
    function fillDescription(activity, data) {
        ...
        if (useOverlay && description.trim()) {
            ...
            // Добавляем подсказку для пульта
            descWrapper.attr('title', 'Нажмите ENTER для просмотра полного описания');
        } else {
            descWrapper.off('hover:enter').removeClass('selector').removeAttr('tabindex');
        }
    }
    ...
    function createDescriptionOverlay(activity, data) {
        ...
        // Добавляем обработчик закрытия на кнопку
        overlay.find('.applecation-description-overlay__close').on('hover:enter', function() {
            closeDescriptionOverlay();
        });
        
        // Регистрируем элемент для управления через пульт
        if (Lampa.Controller) {
    ...
    function showFullDescription() {
        ...
        // Устанавливаем активный оверлей
        window.applecationActiveOverlay = overlay;
        ...
        setTimeout(() => {
            overlay.find('.applecation-description-overlay__close').focus();
            
            // Добавляем подсказку для пользователя
            if (Lampa.Noty && typeof Lampa.Noty.show === 'function') {
                Lampa.Noty.show('Используйте кнопку НАЗАД на пульте для закрытия описания', 3000);
            }
        }, 100);
    }

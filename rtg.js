...
(function() {
  ...
  */
  ...
  var RCFG_DEFAULT = {
    ...
    ratings_rating_border: false  // Нова опція для обводки рейтингів
  };
  ...
  */
  ...
  /**
   * Отримує колір для рейтингу (з int.js)
   */
  ...
  /**
   * Застосовує колір та обводку до рейтингу (з int.js)
   */
  function siStyleApplyColorByRating(element) {
    ...
    var cfg = getCfg();
    if (color && cfg.colorizeAll) {
      $el.css("color", color);

      if (cfg.ratingBorder && !$el.hasClass("card__vote")) {
  ...
  /**
   * Оновлює кольори всіх рейтингів на сторінці (з int.js)
   */
  ...
  */
  ...
  function updateHiddenElements(data) {
    ...
    if (imdbContainer.length) {
      ...
      if (!cfg.enableImdb || !data.imdb_display) {
        ...
        // Застосовуємо обводку, якщо потрібно
        if (cfg.colorizeAll && cfg.ratingBorder) {
          siStyleApplyColorByRating(imdbContainer.find('> div').eq(0));
        }
      }
    }
    ...
    if (tmdbContainer.length) {
      ...
      if (!cfg.enableTmdb || !data.tmdb_display) {
        ...
        // Застосовуємо обводку, якщо потрібно
        if (cfg.colorizeAll && cfg.ratingBorder) {
          siStyleApplyColorByRating(tmdbContainer.find('> div').eq(0));
        }
      }
    }
  }
  ...
  function insertRatings(data) {
    ...
    (function() {
      ...
      if (cfg.colorizeAll) {
        cont.addClass(getRatingClass(mcVal));
        // Застосовуємо обводку, якщо потрібно
        if (cfg.ratingBorder) {
          siStyleApplyColorByRating(cont.find('> div').eq(0));
        }
      }
    })();
    ...
    (function() {
      ...
      cont.removeClass('rating--green rating--blue rating--orange rating--red');
      if (cfg.colorizeAll) {
        cont.addClass(getRatingClass(rtVal));
        // Застосовуємо обводку, якщо потрібно
        if (cfg.ratingBorder) {
          siStyleApplyColorByRating(cont.find('> div').eq(0));
        }
    ...
    (function() {
      ...
      cont.removeClass('rating--green rating--blue rating--orange rating--red');
      if (cfg.colorizeAll) {
        cont.addClass(getRatingClass(pcVal));
        // Застосовуємо обводку, якщо потрібно
        if (cfg.ratingBorder) {
          siStyleApplyColorByRating(cont.find('> div').eq(0));
        }
  ...
  function calculateAverageRating(data) {
    ...
    // Застосовуємо обводку до середнього рейтингу, якщо потрібно
    if (cfg.colorizeAll && cfg.ratingBorder) {
      siStyleApplyColorByRating(avgElement.find('> div').eq(0));
    }

    try {
      applyAwardsColor(rateLine, getCfg());
    } catch (e) {}
  ...
  */
  ...
  function fetchAdditionalRatings(card) {
    ...
    function renderAll() {
      ...
      applyStylesToAll();
      // Застосовуємо кольори та обводку до всіх рейтингів
  ...
  */
  ...
  function getCfg() {
    ...
    var ratingBorder = !!Lampa.Storage.field('ratings_rating_border', RCFG_DEFAULT.ratings_rating_border);
  ...
  function ensureDefaultToggles() {
    ...
    if (typeof Lampa.Storage.get('ratings_rating_border') === 'undefined') {
      Lampa.Storage.set('ratings_rating_border', false);
    }
  }
  ...
  function addSettingsSection() {
    ...
    Lampa.SettingsApi.addParam({
      component: 'lmp_ratings',
      param: {
        name: 'ratings_rating_border',
        type: 'trigger',
        "default": RCFG_DEFAULT.ratings_rating_border
      },
      field: {
        name: 'Обведення рейтингів',
        description: 'Додає обводку навколо рейтингів'
      },
      onRender: function() {}
    });

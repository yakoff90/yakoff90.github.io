(function () {
    ...
    // ============================================================
    // ===  МІТКА ТИПУ КОНТЕНТУ (СЕРІАЛ / ФІЛЬМ)                ===
    // ============================================================

    function createTypeBadge(cardEl, cardData) {
        if (!cardEl || !cardData) return;

        // Визначаємо тип
        var type = getMediaType(cardData);
        ...
        // Прибираємо стару мітку, якщо є
        var old = view.querySelector('.card--type');
        if (old) old.remove();

        // Створюємо нову мітку
        var badge = document.createElement('div');
        badge.className = 'card--type ' + (type === 'tv' ? 'tv' : 'movie');
        badge.innerHTML = '<div>' + (type === 'tv' ? 'СЕРІАЛ' : 'ФІЛЬМ') + '</div>';
        view.appendChild(badge);
        setTimeout(function(){badge.classList.add('show');}, 100);
    }

    // Додаємо стилі для мітки типу контенту

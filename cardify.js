        ...
        _createClass(LFUCache, [
            ...
            },
            {
                ...
                value: function skodf(e) {
                    ...
                    // АБСОЛЮТНЕ ФІКСУВАННЯ СЛОГАНУ - ПРЯМО ПІД НАЗВОЮ
                    var titleElement = render.find('.full-start-new__title');
                    
                    // Видаляємо ВСІ можливі слогани з будь-яких місць
                    render.find('.full-start-new__tagline, [class*="tagline"], [class*="slogan"]').remove();
                    
                    // Якщо є дані про слоган
                    if (e.data && e.data.movie && e.data.movie.tagline) {
                        // Створюємо слоган і вставляємо його БЕЗПОСЕРЕДНЬО ПІСЛЯ ЗАГОЛОВКА
                        var taglineHtml = $('<div class="full-start-new__tagline cardify-fixed-tagline">' + e.data.movie.tagline + '</div>');
                        
                        // Вставляємо відразу після заголовка
                        if (titleElement.length) {
                            titleElement.after(taglineHtml);
                        } else {
                            // Якщо заголовка немає, додаємо на початок
                            render.find('.cardify__left').prepend(taglineHtml);
                        }
                    }

                    // ПРИМУСОВО застосовуємо стилі через JavaScript кожні 100мс
                    var protectTagline = function() {
                        var tagline = render.find('.cardify-fixed-tagline, .full-start-new__tagline');
                        if (tagline.length) {
                            tagline.css({
                                'position': 'relative',
                                'display': 'block',
                            ...
                            // Перевіряємо, чи слоган все ще після заголовка
                            if (titleElement.length && tagline.prev().length && !tagline.prev().is(titleElement)) {
                                titleElement.after(tagline);
                            }
                        }
                    };
                    
                    // Запускаємо захист кілька разів
                    protectTagline();
                    setTimeout(protectTagline, 100);
                    setTimeout(protectTagline, 500);
                    setTimeout(protectTagline, 1000);
                    
                    // Постійний захист кожні 200мс протягом 5 секунд
                    var protectInterval = setInterval(protectTagline, 200);
                    setTimeout(function() { clearInterval(protectInterval); }, 5000);

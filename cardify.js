        ...
        _createClass(LFUCache, [
            ...
            },
            {
                ...
                value: function skodf(e) {
                    ...
                    // ВІДНОВЛЕННЯ КНОПОК ВІДТВОРЕННЯ
                    this.restorePlayButtons(e, render);
            ...
            {
                key: "restorePlayButtons",
                value: function restorePlayButtons(e, render) {
                    var _this = this;
                    
                    // Знаходимо кнопки відтворення
                    var playButton = render.find('.button--play');
                    var torrentButton = render.find('.view--torrent');
                    var trailerButton = render.find('.view--trailer');
                    
                    // Якщо кнопки існують, відновлюємо їх функціонал
                    if (playButton.length) {
                        playButton.off('click').on('click', function() {
                            _this.handlePlayButton(e);
                        });
                        
                        // Додаємо обробник для клавіші Enter
                        playButton.off('keydown').on('keydown', function(event) {
                    ...
                    if (torrentButton.length) {
                        torrentButton.removeClass('hide').off('click').on('click', function() {
                            _this.handleTorrentButton(e);
                        });
                        
                        torrentButton.off('keydown').on('keydown', function(event) {
                            if (event.keyCode === 13) { // Enter
                                _this.handleTorrentButton(e);
                            }
                        });
                    ...
                    if (trailerButton.length) {
                        trailerButton.removeClass('hide').off('click').on('click', function() {
                            _this.handleTrailerButton(e);
                        });
                        
                        trailerButton.off('keydown').on('keydown', function(event) {
                            if (event.keyCode === 13) { // Enter
                                _this.handleTrailerButton(e);
                            }
                        });
                    ...
                    // Додаємо обробники для стандартних кнопок Lampa
                    setTimeout(function() {
                        _this.setupLampaButtons(e, render);
                    }, 100);
                }
            },
            {
                key: "setupLampaButtons",
                value: function setupLampaButtons(e, render) {
                    // Знаходимо всі кнопки з класом selector
                    var buttons = render.find('.selector');
                    ...
                    buttons.each(function() {
                        ...
                        if (button.hasClass('button--play')) {
                            buttonType = 'play';
                        } else if (button.hasClass('view--torrent')) {
                            buttonType = 'torrent';
                        } else if (button.hasClass('view--trailer')) {
                            buttonType = 'trailer';
                        } else if (button.hasClass('button--book')) {
                            buttonType = 'bookmark';
                        } else if (button.hasClass('button--reaction')) {
                            buttonType = 'reaction';
                        } else if (button.hasClass('button--subscribe')) {
                            buttonType = 'subscribe';
                        } else if (button.hasClass('button--options')) {
                            buttonType = 'options';
                        }
                        
                        if (buttonType) {
                            button.attr('data-button-type', buttonType);
                            
                            // Додаємо фокус для TV
                    ...
                    // Налаштовуємо Controller для обробки кнопок
                    this.setupController(e);
                }
            },
            {
                key: "setupController",
                value: function setupController(e) {
                    var _this = this;
                    
                    // Додаємо обробник для контролера
                    if (Lampa.Controller && Lampa.Controller.enabled) {
                        var originalController = Lampa.Controller.enabled();
                        
                        if (originalController && originalController.name === 'full_start') {
                            // Клонуємо оригінальний контролер
                            var controllerCopy = Object.assign({}, originalController);
                            
                            // Додаємо обробники для кнопок
                            controllerCopy.enter = function() {
                                var focused = $('.selector.focus');
                                if (focused.length) {
                                    var buttonType = focused.attr('data-button-type');
                                    ...
                                    switch(buttonType) {
                                        ...
                                        case 'trailer':
                                            ...
                                            break;
                                        case 'bookmark':
                                            _this.handleBookmarkButton(e);
                                            break;
                                        case 'reaction':
                                            _this.handleReactionButton(e);
                                            break;
                                        case 'subscribe':
                                            _this.handleSubscribeButton(e);
                                            break;
                                        case 'options':
                                            _this.handleOptionsButton(e);
                                            break;
                                    }
                                }
                                return originalController.enter ? originalController.enter() : null;
                            };
                            
                            // Замінюємо контролер
                            Lampa.Controller.add('full_start_cardify', controllerCopy);
                            Lampa.Controller.toggle('full_start_cardify');
                        }
                    }
                }
            },
            {
                ...
                value: function handlePlayButton(e) {
                    console.log('Cardify: Play button clicked');
                    
                    // Отримуємо дані про контент
                    var card = e.object.card || e.data.movie || e.data.tv;
                    var component = e.object.activity.component;
                    
                    if (component && component.start) {
                        // Викликаємо стандартний метод відтворення Lampa
                        component.start();
                    } else {
                        // Альтернативний спосіб - шукаємо стандартну кнопку відтворення
                        var playBtn = $('.button--play, .full-start__button--play, [data-action="play"]').first();
                        if (playBtn.length) {
                            playBtn.trigger('click');
                        } else {
                            // Якщо нічого не знайшли, спробуємо запустити через API Lampa
                            if (Lampa.Player && Lampa.Player.play) {
                                Lampa.Player.play(card);
                            }
                        }
                    }
                }
            ...
            {
                key: "handleTorrentButton",
                value: function handleTorrentButton(e) {
                    console.log('Cardify: Torrent button clicked');
                    
                    var card = e.object.card || e.data.movie || e.data.tv;
                    
                    // Шукаємо стандартну кнопку торрентів
                    var torrentBtn = $('.view--torrent, [data-action="torrent"]').first();
                    if (torrentBtn.length) {
                        torrentBtn.trigger('click');
                    } else {
                        // Спробуємо відкрити торрент меню через Lampa
                        if (Lampa.Activity && Lampa.Activity.start) {
            ...
            {
                key: "handleTrailerButton",
                value: function handleTrailerButton(e) {
                    console.log('Cardify: Trailer button clicked');
                    
                    // Якщо трейлер вже відтворюється, просто показуємо його
                    if (window.cardify_trailer_active) {
                        return;
                    }
                    
                    var videoData = this.getVideoData(e);
                    if (videoData) {
                        new Trailer(e.object, videoData);
                    } else {
                        // Якщо трейлера немає, показуємо повідомлення
                        Lampa.Noty.show(Lampa.Lang.translate('trailer_not_found'));
                    }
                }
            },
            {
                key: "handleBookmarkButton",
                value: function handleBookmarkButton(e) {
                    console.log('Cardify: Bookmark button clicked');
                    // Стандартна обробка кнопки закладок
                    var bookmarkBtn = $('.button--book, [data-action="bookmark"]').first();
            ...
            {
                key: "handleReactionButton",
                value: function handleReactionButton(e) {
                    console.log('Cardify: Reaction button clicked');
                    // Стандартна обробка кнопки реакцій
                    var reactionBtn = $('.button--reaction, [data-action="reaction"]').first();
                    if (reactionBtn.length) {
                        reactionBtn.trigger('click');
                    }
                }
            },
            {
                key: "handleSubscribeButton",
                value: function handleSubscribeButton(e) {
                    console.log('Cardify: Subscribe button clicked');
                    // Стандартна обробка кнопки підписки
                    var subscribeBtn = $('.button--subscribe, [data-action="subscribe"]').first();
            ...
            {
                key: "handleOptionsButton",
                value: function handleOptionsButton(e) {
                    console.log('Cardify: Options button clicked');
                    // Стандартна обробка кнопки опцій
                    var optionsBtn = $('.button--options, [data-action="options"]').first();
            ...
            {
                key: "getVideoData",
                value: function getVideoData(e) {
                    // Функція для отримання даних про відео (вже є в коді)
                    if (e.data && e.data.videos && e.data.videos.results && e.data.videos.results.length) {
                        var items = [];
                        e.data.videos.results.forEach(function (element) {
                            items.push({

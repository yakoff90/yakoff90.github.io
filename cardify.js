        ...
        _createClass(LFUCache, [
            ...
            },
            {
                ...
                value: function skodf(e) {
                    ...
                    // Затримка для покращення завантаження
                    setTimeout(function() {
                        ...
                        if (originalTagline.length && originalTagline.is(':hidden')) {
                            // Показуємо його знову зі стилями - БІЛЬШИЙ ВІДСТУП ЗВЕРХУ (1.5em)
                            originalTagline.show().css({
                                'font-size': '1.7em',
                                'font-style': 'italic',
                                'color': 'rgba(255,255,255,0.7)',
                                'margin': '1.5em 0 0.5em 0'  // ЗМІНЕНО: більший відступ зверху
                            });
                        }
                        
                        // Якщо слоган відсутній у DOM, але є в даних
                        if (!originalTagline.length && e.data && e.data.movie && e.data.movie.tagline) {
                            // Створюємо елемент з класами як у оригіналі та стилями - БІЛЬШИЙ ВІДСТУП ЗВЕРХУ
                            var taglineHtml = '<div class="full-start-new__tagline" style="font-size:1.7em; font-style:italic; color:rgba(255,255,255,0.7); margin:1.5em 0 0.5em 0;">' + 
                        ...
                        // Фікс для кнопок - переконуємося, що вони доступні
                        setTimeout(function() {
                            var buttons = render.find(".full-start__button");
                            buttons.css('opacity', '1');
                            buttons.css('visibility', 'visible');
                        }, 100);
                    }, 50); // Невелика затримка для стабілізації DOM
            ...
            },
            {
                ...
                value: function loadOriginalPoster(e, render) {
                    ...
                    if (backdropPath && bgImg.length) {
                        // Використовуємо завантаження з попереднім завантаженням
                        var originalUrl = "https://image.tmdb.org/t/p/original" + backdropPath;
                        var tempImg = new Image();
                        
                        // Додаємо клас для покращення відображення
                        bgImg.addClass("cardify-poster-loading");
                        
                        tempImg.onload = function () {
                            bgImg.attr("src", originalUrl);
                            bgImg.removeClass("cardify-poster-loading");
                            bgImg.addClass("cardify-poster-loaded");
                        };

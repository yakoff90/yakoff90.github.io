        ...
        _createClass(LFUCache, [
            ...
            },
            {
                ...
                value: function skodf(e) {
                    ...
                    // ЗБЕРІГАЄМО ОРИГІНАЛЬНИЙ СЛОГАН (TAGLINE) З НОВИМИ СТИЛЯМИ - ТІЛЬКИ ПІД ЛОГОТИПОМ
                    ...
                    if (originalTagline.length && originalTagline.is(':hidden')) {
                        // Показуємо його знову з новими стилями - завжди під логотипом
                    ...
                    // Якщо слоган відсутній у DOM, але є в даних - додаємо його безпосередньо під логотип
                    if (!originalTagline.length && e.data && e.data.movie && e.data.movie.tagline) {
                        // Створюємо контейнер для слогану з класами як у оригіналі та новими стилями
                        var taglineHtml = '<div class="full-start-new__tagline" style="font-size: 1.7em; font-style: italic; color: rgba(255,255,255,0.8); margin: 0.5em 0 1em 0; position: relative; clear: both; width: 100%; display: block;">' + 
                                         e.data.movie.tagline + '</div>';
                        
                        // Додаємо слоган безпосередньо після заголовка (під логотипом)
                        var titleElement = render.find('.full-start-new__title');
                        if (titleElement.length) {
                            titleElement.after(taglineHtml);
                        } else {
                            // Якщо заголовок не знайдено, додаємо на початок правої частини
                            render.find('.full-start-new__right .cardify__left').prepend(taglineHtml);
                        }
                    }

                    // Переконуємося, що слоган завжди залишається під логотипом і не зміщується іншими елементами
                    var tagline = render.find('.full-start-new__tagline');
                    if (tagline.length) {
                        // Видаляємо будь-які попередні стилі, які можуть впливати на позицію
                        tagline.css({
                            'position': 'relative',
                            'clear': 'both',
                            'width': '100%',
                        ...
                        // Переконуємося, що слоган знаходиться безпосередньо після заголовка
                        var titleElement = render.find('.full-start-new__title');
                        if (titleElement.length && tagline.parent().is(titleElement.parent())) {
                            // Якщо слоган не знаходиться відразу після заголовка, переміщуємо його
                            if (tagline.prev().length && !tagline.prev().is(titleElement)) {
                                titleElement.after(tagline);
                            }
                        }
                    }

(function () {
    ...
    try {
        seasonsCache = JSON.parse(Lampa.Storage.get('season_cache_ua_v4', '{}'));
        flagCache = JSON.parse(Lampa.Storage.get('flag_cache_ua_v4', '{}'));
    } catch (e) {}
    ...
    // Функція для отримання кольору рейтингу (кольорова)
    function getRatingColor(rating) {
        if (rating >= 0 && rating <= 3) return 'rgba(231, 76, 60, 0.9)';      // червоний
        if (rating > 3 && rating <= 5) return 'rgba(230, 126, 34, 0.9)';      // помаранчевий
        if (rating > 5 && rating <= 6.5) return 'rgba(241, 196, 15, 0.9)';     // жовтий
        if (rating > 6.5 && rating < 8) return 'rgba(52, 152, 219, 0.9)';      // синій
        if (rating >= 8 && rating <= 10) return 'rgba(46, 204, 113, 0.9)';     // зелений
        return 'rgba(0, 0, 0, 0.7)';                                           // чорний (запасний)
    }
            ...
            requestQueue.add(function() {
                ...
                Lampa.Network.silent(url, 
                    function(response) {
                        ...
                        try {
                            Lampa.Storage.set('flag_cache_ua_v4', JSON.stringify(flagCache));
                        } catch (e) {}
    ...
    function getSeasonData(tmdbId, callback) {
        ...
        fetch(PROXIES[0] + url)
            ...
            .then(function(data) {
                ...
                try {
                    Lampa.Storage.set('season_cache_ua_v4', JSON.stringify(seasonsCache));
                } catch (e) {}
                callback(data);
            })
    ...
    function addBadges(cardElement, movieData) {
        ...
        // 1. РІК (верхній правий кут) - ЧОРНИЙ
        ...
        // 2. РЕЙТИНГ (нижній правий кут) - КОЛЬОРОВИЙ
        ...
        // 4. СЕЗОН (верхній лівий кут) - ЧОРНИЙ (тільки для серіалів)
    ...
    style.innerHTML = `
        ...
        /* РЕЙТИНГ - нижній правий (КОЛЬОРОВИЙ - фон задається JavaScript) */
        .badge-rating {
            right: 0 !important;
            bottom: 0 !important;
        ...
        /* ===== ВИДАЛЕНО ЗАТЕМНЕННЯ ФОНУ ===== */
        /* Більше немає card__view::after з затемненням */

        /* Фокус для карток - тільки рамка, без зміни яскравості */

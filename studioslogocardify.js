(function () {
  ...
  Lampa.Listener.follow('full', function(e) {
    ...
    if (isCardify) {
      // Для Cardify шаблону - додаємо логотипи в правий нижній кут
      var rightSection = $('.cardify__right');
      if (rightSection.length) {
        // Видаляємо попередні логотипи, якщо вони є
        rightSection.find('.cardify-studios-logos').remove();
        
        // Створюємо контейнер для логотипів
        var studiosContainer = $('<div class="cardify-studios-logos"></div>');
        var studioHtml = getStudioLogos(e.data.movie);
        
        if (studioHtml) {
          studiosContainer.html(studioHtml);
          rightSection.append(studiosContainer);
        }
      }
    } else {

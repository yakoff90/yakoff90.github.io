...
var style='<style id="maxsm_omdb_rating">.full-start-new__rate-line{visibility:hidden;flex-wrap:wrap;gap:.4em 0}.full-start-new__rate-line>*{margin-left:0!important;margin-right:.6em!important}.rate--avg.rating--green{color:#4caf50}.rate--avg.rating--lime{color:#3399ff}.rate--avg.rating--orange{color:#ff9933}.rate--avg.rating--red{color:#f44336}.rate--oscars{color:gold}</style>';
...
function insertRatings(rt,mc,osc){var render=getRender();if(!render)return;var rateLine=$('.full-start-new__rate-line',render);if(!rateLine.length)return;var lastRate=$('.full-start__rate:last',rateLine);try{if(rt&&!isNaN(rt)&&!$('.rate--rt',rateLine).length){var rv=rt.toFixed(1);var el=$('<div class="full-start__rate rate--rt"><div>'+rv+'</div><div class="source--name"></div></div>');el.find('.source--name').html(Lampa.Lang.translate('source_rt'));if(lastRate.length)el.insertAfter(lastRate);else rateLine.prepend(el)}if(mc&&!isNaN(mc)&&!$('.rate--mc',rateLine).length){var mv=mc.toFixed(1);var ins=$('.rate--rt',rateLine).length?$('.rate--rt',rateLine):lastRate;var el2=$('<div class="full-start__rate rate--mc"><div>'+mv+'</div><div class="source--name"></div></div>');el2.find('.source--name').html(Lampa.Lang.translate('source_mc'));if(ins.length)el2.insertAfter(ins);else rateLine.prepend(el2)}if(osc&&!isNaN(osc)&&osc>0&&!$('.rate--oscars',rateLine).length){var el3=$('<div class="full-start__rate rate--oscars"><div>'+osc+'</div><div class="source--name"></div></div>');el3.find('.source--name').html(Lampa.Lang.translate('maxsm_omdb_oscars'));rateLine.prepend(el3)}}catch(e){}}

function calculateAverageRating(){var render=getRender();if(!render)return;var rateLine=$('.full-start-new__rate-line',render);if(!rateLine.length)return;var ratings={imdb:parseFloat($('.rate--imdb div:first',rateLine).text())||0,tmdb:parseFloat($('.rate--tmdb div:first',rateLine).text())||0,mc:parseFloat($('.rate--mc div:first',rateLine).text())||0,rt:parseFloat($('.rate--rt div:first',rateLine).text())||0};var totalWeight=0,weightedSum=0,ratingsCount=0;for(var k in ratings){if(ratings.hasOwnProperty(k)&&!isNaN(ratings[k])&&ratings[k]>0){weightedSum+=ratings[k]*WEIGHTS[k];totalWeight+=WEIGHTS[k];ratingsCount++}}$('.rate--avg',rateLine).remove();if(ratingsCount>1&&totalWeight>0){var avg=weightedSum/totalWeight;var cls=getRatingClass(avg);var avgEl=$('<div class="full-start__rate rate--avg '+cls+'"><div>'+avg.toFixed(1)+'</div><div class="source--name">'+Lampa.Lang.translate("ratimg_omdb_avg")+'</div></div>');$('.full-start__rate:first',rateLine).before(avgEl)}removeLoadingAnimation();rateLine.css('visibility','visible')}

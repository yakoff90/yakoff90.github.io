...
(function () {
  ...
  // Використовуємо суворий режим для кращої якості коду та запобігання помилок.
  ...
  var LTF_CONFIG = window.LTF_CONFIG || {
    BADGE_STYLE: 'text',
    // 'text' | 'flag_count' | 'flag_only'
    SHOW_FOR_TV: true,
    // показувати на серіалах
    // --- Налаштування кешу ---
    CACHE_VERSION: 4,
    // ❗ Змініть це число (напр. 5), якщо хочете примусово скинути весь кеш у користувачів.
    CACHE_KEY: 'lampa_ukr_tracks_cache',
    // Унікальний ключ для зберігання кешу в LocalStorage.
    CACHE_VALID_TIME_MS: 24 * 60 * 60 * 1000,
    // Час життя кешу (24 години). Після цього він вважається недійсним.
    CACHE_REFRESH_THRESHOLD_MS: 12 * 60 * 60 * 1000,
    // Через скільки часу кеш потребує фонового оновлення (22 годин).

    // --- Налаштування логування для налагодження ---
    LOGGING_GENERAL: false,
    // Загальні логи (старт плагіна, оновлення мережі).
    LOGGING_TRACKS: false,
    // Логи пошуку (що шукаємо, що знайшли, фільтрація).
    LOGGING_CARDLIST: false,
    // Логи обробки карток (скільки карток в пачці, тощо).

    // --- Налаштування API та мережі ---
    JACRED_PROTOCOL: 'http://',
    // Протокол для API JacRed.
    JACRED_URL: 'jacred.xyz',
    // Домен API JacRed.
    PROXY_LIST: [
    // Список проксі-серверів для обходу CORS-обмежень.
    'http://api.allorigins.win/raw?url=', 'http://cors.bwa.workers.dev/'],
    PROXY_TIMEOUT_MS: 3500,
    // Максимальний час очікування відповіді від одного проксі (3.5 секунди).
    MAX_PARALLEL_REQUESTS: 10,
    // Максимальна кількість одночасних запитів до API.
    MAX_RETRY_ATTEMPTS: 2,
    // (Зараз не використовується, але зарезервовано).

    // --- Налаштування функціоналу ---
    SHOW_TRACKS_FOR_TV_SERIES: true,
    // Чи показувати мітки для серіалів (true або false).

    // --- ✅ ОНОВЛЕНО: Налаштування відображення ---
    DISPLAY_MODE: 'flag_count',
    // Режим відображення мітки. Варіанти:
    // 'text': "Ukr" або "2xUkr"
    ...
    MANUAL_OVERRIDES: {
      ...
      //✅Примусово показувати Ukr для цього ID
      '1195518': {
        track_count: 2
      },
      //✅Примусово показувати 2xUkr для цього ID
      '215995': {
        track_count: 2
      },
      //✅Примусово показувати 2xUkr для цього ID
      ...
      //✅Примусово показувати 3xUkr для цього ID
      '245827': {
        track_count: 0
      } //✅Примусово не показувати Ukr для цього ID
  ...
  ".card__tracks {" + " position: absolute !important; " +
  ...
  // Високий z-index, щоб бути поверх інших елементів
  " width: fit-content !important; " +
  // Ширина за вмістом
  " max-width: calc(100% - 1em) !important; " +
  // Максимальна ширина
  " overflow: hidden !important;" +
  // Приховувати все, що виходить за межі
  "}" +
  ...
  // Зміщуємо мітку нижче рейтингу
  "}" +
  // Стиль для внутрішнього `div`, що містить текст або SVG
  ".card__tracks div {" + " text-transform: none !important; " +
  // Без перетворення у великі літери
  ...
  // Заборона переносу рядка
  " display: flex !important; " +
  // Flex-контейнер для вирівнювання (напр. "2x" і "[SVG]")
  " align-items: center !important; " +
  // Вертикальне вирівнювання
  " gap: 4px !important; " +
  // Відстань між елементами (між "2x" і "[SVG]")
  " text-shadow: 0.5px 0.5px 1px rgba(0,0,0,0.3) !important; " +
  // Тінь для тексту
  "}" + /* Стилі CSS для прапора*/
  ".card__tracks .flag-css {" + " display: inline-block;" + " width: 1.5em;" + " height: 0.8em;" + " vertical-align: middle;" +
  ...
  // Гарантуємо відсутність стандартної рамки
  ...
  // Темно-синій (верхній край)
  "inset 0px -1px 0px 0px #D0A800;" +
  // Темно-жовтий (нижній край)
  ...
  function getBestReleaseWithUkr(normalizedCard, cardId, callback) {
    ...
    enqueueTask(function (done) {
      ...
      function searchJacredApi(searchTitle, searchYear, apiCallback) {
        ...
        var apiUrl = LTF_CONFIG.JACRED_PROTOCOL + LTF_CONFIG.JACRED_URL + '/api/v1.0/torrents?search=' + encodeURIComponent(searchTitle) + '&year=' + searchYear + '&uid=' + userId;
  ...
  function updateCardListTracksElement(cardView, trackCount) {
    ...
    function ensurePositionClass(el) {
      ...
      if (topStyle !== 'auto' && parseInt(topStyle) < 100) el.classList.add('positioned-below-rating');else el.classList.remove('positioned-below-rating');
    }
  ...
  function reprocessVisibleCardsChunked() {
    const cards = Array.from(document.querySelectorAll('.card')).filter(c => c.isConnected && document.body.contains(c));
    const BATCH = 20,
      DELAY = 25;
    let i = 0;
    (function tick() {
  ...
  (function (_Lampa2) {
    ...
    function start() {
      ...
      st = load();
      apply();
      if ((_Lampa = Lampa) !== null && _Lampa !== void 0 && (_Lampa = _Lampa.SettingsApi) !== null && _Lampa !== void 0 && _Lampa.addParam) {
        // !!! ЗАСТОСУВАТИ ЗМІНУ ТУТ: обгортаємо виклик у setTimeout(..., 0)
        setTimeout(registerUI, 0);
      }
    }
    if (window.appready) start();else if ((_Lampa2 = Lampa) !== null && _Lampa2 !== void 0 && _Lampa2.Listener) Lampa.Listener.follow('app', e => {

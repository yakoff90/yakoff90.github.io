(function () {
  ...
  Lampa.Lang.add({
    ...
    interface_mod_new_colored_buttons_desc: {
      en: 'Replace icons with custom ones: Torrents, Online (BwaRC/Cinema), Trailers. Update button text',
      uk: 'Замінює іконки на кастомні: Торенти, Онлайн (BwaRC/Cinema), Трейлери. Оновлює текст кнопок'
    },
  ...
  /**
   * Об'єкт з поточними налаштуваннями плагіна
   */
  var settings = {
          ...
          switch (key) {
            ...
            case 'interface_mod_new_colored_buttons':
              ...
              if (settings.colored_buttons) {
                setTimeout(initColoredButtonsSystem, 100);
              } else {
                setColoredButtonsEnabled(false);
              }
              break;
  ...
  function rebuildButtonsNow() {
    ...
    if (settings.colored_buttons) {
      setTimeout(function() {
        if (window.__ifx_colbtn && window.__ifx_colbtn.isInitialized) {
  ...
  /* ============================================================
   * КОЛЬОРОВІ КНОПКИ (ЯК У PLUGIN ColoredButtons.js)
   * ============================================================ */
  var __ifx_colbtn = {
    ...
    // SVG джерела (inline для надійності на TV)
    TORRENT_SVG_SOURCE: `
...
<svg height="70" viewBox="0 0 80 70" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" clip-rule="evenodd" d="M71.2555 2.08955C74.6975 3.2397 77.4083 6.62804 78.3283 10.9306C80 18.7291 80 35 80 35C80 35 80 51.2709 78.3283 59.0694C77.4083 63.372 74.6975 66.7603 71.2555 67.9104C65.0167 70 40 70 40 70C40 70 14.9833 70 8.74453 67.9104C5.3025 66.7603 2.59172 63.372 1.67172 59.0694C0 51.2709 0 35 0 35C0 35 0 18.7291 1.67172 10.9306C2.59172 6.62804 5.3025 3.2395 8.74453 2.08955C14.9833 0 40 0 40 0C40 0 65.0167 0 71.2555 2.08955ZM55.5909 35.0004L29.9773 49.5714V20.4286L55.5909 35.0004Z" fill="currentColor"/>
    ...
    // Розміри іконок для телевізорів
    ICON_SIZES: {
      tv: {
        width: '30',
        height: '30'
  ...
  function injectColoredButtonsCss() {
    ...
    var css = `
      ...
      .full-start__button.view--reyohoho_mod svg path,
      .full-start__button.view--online_mod svg path { 
        fill: #f44336 !important; 
      }
      
      /* Анімації для TV (простіші) */
      .full-start__button {
      ...
      /* Кастомні класи для замінених іконок */
      ...
      /* Стилі для тексту кнопок */
      .full-start__button.view--torrent span {
        color: #4caf50 !important;
      }
      .full-start__button.view--online span {
        color: #2196f3 !important;
      }
      .full-start__button.view--reyohoho_mod span,
      .full-start__button.view--online_mod span {
  ...
  /**
   * Отримує розмір іконки для поточного пристрою
   */
  function getIconSize() {
    // Для телевізора завжди використовуємо TV розміри
    return __ifx_colbtn.ICON_SIZES.tv;
  }

  /**
   * Створює SVG елемент з рядка
   */
  ...
  function replaceIconPreservingAttrs(origSvg, newSvgSource, options) {
    try {
      ...
      // Застосовуємо кастомні налаштування
      if (options) {
        ...
        // Розміри за замовчуванням для TV
        fresh.setAttribute('width', iconSize.width);
        fresh.setAttribute('height', iconSize.height);
      }

      // Для TV важливо зберегти viewBox
      if (!fresh.getAttribute('viewBox') && origSvg.getAttribute('viewBox')) {
        fresh.setAttribute('viewBox', origSvg.getAttribute('viewBox'));
      }
  ...
  /**
   * Отримує назву плагіна з кнопки
   */
  ...
  /**
   * Додає обробники подій для кнопок (адаптовано для TV)
   */
  function attachHoverEnter(btn) {
    if (btn.classList.contains('hover-enter-attached')) return;
    
    // Для TV використовуємо focus замість hover
    btn.addEventListener('focus', function (e) {
      __ifx_colbtn.lastActiveButton = btn;
    });
   ...
   */
  function processColoredButtons() {
    if (!__ifx_colbtn.isInitialized) return;
    ...
    torrentButtons.forEach(function (btn) {
      ...
      if (svg) {
        if (replaceIconPreservingAttrs(svg, __ifx_colbtn.TORRENT_SVG_SOURCE, {
          ...
          if (span && span.textContent !== 'Торенти') {
            span.textContent = 'Торенти';
          }
        }
      }
    });

    // 2. Онлайн-кнопки (тільки BwaRC і Cinema)
    if (__ifx_colbtn.ONLINE_SVG_SOURCE) {
      ...
      onlineButtons.forEach(function (btn) {
        ...
        // Для BwaRC - міняємо іконку і текст
        if (pluginName.toLowerCase().includes('bwa')) {
          setTimeout(function() {
            ...
            if (svg && !svg.classList.contains('custom-svg-replaced')) {
              if (replaceIconPreservingAttrs(svg, __ifx_colbtn.ONLINE_SVG_SOURCE, {
                width: iconSize.width,
                height: iconSize.height
              })) {
        ...
        // Інші плагіни - просто позначаємо
        else {
          btn.classList.add('online-svg-applied');
        }
      });
    ...
    // 3. Кнопки reyohoho_mod
    if (__ifx_colbtn.REYOHOHO_SVG_SOURCE) {
      ...
      reyohohoButtons.forEach(function (btn) {
        ...
        if (svg) {
          setTimeout(function() {
            ...
            if (replaceIconPreservingAttrs(svg, __ifx_colbtn.REYOHOHO_SVG_SOURCE, {
              width: iconSize.width,
              height: iconSize.height,
    ...
    // 4. Кнопки online_mod (використовуємо іконку reyohoho)
    if (__ifx_colbtn.REYOHOHO_SVG_SOURCE) {
    ...
    if (count > 0) {
      console.log('✅ Кольорові кнопки оброблено:', count);
    }
  }

  /**
   * Спостерігач для динамічного оновлення кнопок
   */
  function observeForColoredButtons() {
    var mo = new MutationObserver(function (muts) {
      ...
      if (needsUpdate) {
        // Дебаунс для TV (менш часті оновлення)
        clearTimeout(__ifx_colbtn.observerTimeout);
        __ifx_colbtn.observerTimeout = setTimeout(processColoredButtons, 300);
      }
    });
  ...
  /**
   * Ініціалізує систему кольорових кнопок
   */
  function initColoredButtonsSystem() {
    if (__ifx_colbtn.isInitialized) return;
    
    console.log('🚀 Ініціалізація кольорових кнопок для TV');
    
    // Додаємо стилі
    injectColoredButtonsCss();
    
    // Ініціалізуємо
    __ifx_colbtn.isInitialized = true;
    
    // Запускаємо спостерігач
    __ifx_colbtn.observer = observeForColoredButtons();
    
    // Кілька спроб обробки
    setTimeout(processColoredButtons, 100);
    setTimeout(processColoredButtons, 500);
    setTimeout(processColoredButtons, 1000);
    
    // Також обробляємо при відкритті повної картки
    Lampa.Listener.follow('full', function (e) {
      if (e.type === 'complite') {
        setTimeout(processColoredButtons, 200);
      }
    });
    
    console.log('✅ Система кольорових кнопок ініціалізована');
  }

  /**
   * Вмикає/вимикає кольорові кнопки
   */
  function setColoredButtonsEnabled(enabled) {
    if (enabled) {
      initColoredButtonsSystem();
    } else {
      // При вимиканні - видаляємо наші стилі та відновлюємо оригінальні іконки
      removeColoredButtonsCss();
      
      // Зупиняємо спостерігач
      if (__ifx_colbtn.observer) {
        __ifx_colbtn.observer.disconnect();
        __ifx_colbtn.observer = null;
      }
      
      __ifx_colbtn.isInitialized = false;
      
      // Видаляємо наші класи з кнопок
      document.querySelectorAll('.full-start__button').forEach(function(btn) {
        btn.classList.remove(
          'utorrent-svg-applied',
          'online-svg-applied', 
          'reyohoho-svg-applied',
          'online-mod-svg-applied',
          'hover-enter-attached'
        );
      });
    }
  }
  ...
  function wireFullCardEnhancers() {
    Lampa.Listener.follow('full', function (e) {
      ...
      setTimeout(function () {
        ...
        // 4. Кольорові кнопки
        if (settings.colored_buttons && __ifx_colbtn.isInitialized) {
          setTimeout(processColoredButtons, 200);
        }
      }, 120);
    });
  ...
  function startPlugin() {
    ...
    if (settings.colored_buttons) {
      // Ініціалізуємо з невеликою затримкою для стабільності на TV
      setTimeout(function() {
        initColoredButtonsSystem();
      }, 1000);
    }

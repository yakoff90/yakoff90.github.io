(function () {
    ...
    function subscribeTracks(data) {
      ...
      // один об'єкт для зберігання "нативних" замін від різних платформ
      var platform_replace = {};
      ...
      // --- Уніфіковані функції для "нативних" платформ (webOS, Tizen тощо)
      function setPlatformTracks(video_tracks) {
        if (inited_parse) {
          var parse_tracks = inited_parse.streams.filter(function (a) {
            return a.codec_type == 'audio';
          });
          log('Tracks', 'platform set tracks:', video_tracks ? video_tracks.length : 0);

          // пробуємо поступово відфільтрувати проблемні кодеки, щоб співпало з нативним масивом
          if (parse_tracks.length !== (video_tracks ? video_tracks.length : 0)) {
            parse_tracks = parse_tracks.filter(function (a) {
          ...
          parse_tracks.forEach(function (track, i) {
            if (video_tracks && video_tracks[i]) {
              // деякі платформи використовують label / language поля
              video_tracks[i].language = track.tags.language;
              video_tracks[i].label = track.tags.title || track.tags.handler_name;
              // додатково можна спробувати записати в title якщо є
              video_tracks[i].title = video_tracks[i].title || video_tracks[i].label;
            }
          });

          // оновимо панель (якщо панель очікує веб-замін)
          try {
            if (parse_tracks.length) Lampa.PlayerPanel.setTracks(parse_tracks.map(function (t, idx) {
              return {
                index: idx,
                language: t.tags.language,
                label: t.tags.title || t.tags.handler_name,
                ghost: false,
                selected: false
              };
            }));
      ...
      function setPlatformSubs(video_subs) {
        if (inited_parse) {
          var parse_subs = inited_parse.streams.filter(function (a) {
            return a.codec_type == 'subtitle';
          });
          log('Tracks', 'platform set subs:', video_subs ? video_subs.length : 0);

          // індексація у різних платформах може відрізнятись (наприклад webOS додає "disabled" елемент на початку)
          if (parse_subs.length !== (video_subs ? video_subs.length : 0) - 1) {
            parse_subs = parse_subs.filter(function (a) {
          ...
          parse_subs.forEach(function (track, a) {
            // для webOS у вас був зсув +1, для інш. платформ — може бути без зсуву, тому пробуємо обидва
            var i = a + 1;
            if (video_subs && video_subs[i]) {
              video_subs[i].language = track.tags.language;
              video_subs[i].label = track.tags.title || track.tags.handler_name;
              video_subs[i].title = video_subs[i].title || video_subs[i].label;
            } else if (video_subs && video_subs[a]) {
              video_subs[a].language = track.tags.language;
              video_subs[a].label = track.tags.title || track.tags.handler_name;
              video_subs[a].title = video_subs[a].title || video_subs[a].label;
            }
          });

          try {
            if (parse_subs.length) Lampa.PlayerPanel.setSubs(parse_subs.map(function (t, idx) {
              return {
                index: idx,
                language: t.tags.language,
                label: t.tags.title || t.tags.handler_name,
      ...
      function canPlay() {
        ...
        // якщо платформа передала свої нативні масиви — спробуємо їх застосувати
        if (platform_replace.subs) setPlatformSubs(platform_replace.subs);else setSubs();
        if (platform_replace.tracks) setPlatformTracks(platform_replace.tracks);else setTracks();
        Lampa.PlayerVideo.listener.remove('canplay', canPlay);
      }

      // обробники подій від різних платформ (webos уже був — лишаємо сумісність)
      function listenWebosSubs(_data) {
        log('Tracks', 'webos subs event');
        platform_replace.subs = _data.subs;
        if (inited_parse) setPlatformSubs(_data.subs);
      }

      function listenWebosTracks(_data) {
        log('Tracks', 'webos tracks event');
        platform_replace.tracks = _data.tracks;
        if (inited_parse) setPlatformTracks(_data.tracks);
      }

      // додаткові слухачі для Tizen (якщо Lampa генерує такі події) — якщо Tizen повертає свої
      function listenTizenSubs(_data) {
        log('Tracks', 'tizen subs event');
        // очікуємо, що _data.subs або просто _data буде масивом
        platform_replace.subs = _data.subs || _data;
        if (inited_parse) setPlatformSubs(platform_replace.subs);
      }

      function listenTizenTracks(_data) {
        log('Tracks', 'tizen tracks event');
        platform_replace.tracks = _data.tracks || _data;
        if (inited_parse) setPlatformTracks(platform_replace.tracks);
      }
      ...
      function listenDestroy() {
        ...
        Lampa.PlayerVideo.listener.remove('tracks', listenTracks);
        Lampa.PlayerVideo.listener.remove('subs', listenSubs);
        // видаляємо усі платформні слухачі
        ...
        Lampa.PlayerVideo.listener.remove('tizen_subs', listenTizenSubs);
        Lampa.PlayerVideo.listener.remove('tizen_tracks', listenTizenTracks);
        Lampa.PlayerVideo.listener.remove('canplay', canPlay);
        log('Tracks', 'end');
      }
      ...
      // додаємо слухачі для Tizen (якщо Lampa/плеєр на Tizen емітить їх)
      Lampa.PlayerVideo.listener.follow('tizen_subs', listenTizenSubs);
      Lampa.PlayerVideo.listener.follow('tizen_tracks', listenTizenTracks);

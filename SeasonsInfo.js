(function () {
    ...
    style.textContent = "    .card--season-complete {\n" +
        "        position: absolute;\n" +
        "        left: 0;\n" +
        "        margin-left: -0.65em;\n" +
        "        bottom: 2.4em;\n" +
    ...
    function initPlugin() {
        ...
        var containersNodeList = document.querySelectorAll('.cards, .card-list, .content, .main, .cards-list, .preview__list');
    ...
    // Секція налаштувань - тепер без стрілкових функцій
    (function () {
        ...
        function load() {
            ...
            return { tmdb_key: s.tmdb_key || '', badge_view: s.badge_view || 'standard' };
        }

        function apply() {
            if (st.tmdb_key) CONFIG.tmdbApiKey = String(st.tmdb_key).trim();
            CONFIG.badgeView = st.badge_view || 'standard';
        }
        ...
        function clearCache() {
            try {
                if (window.safeStorage && safeStorage.removeItem) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onRender: function (item) {
                    try {
                        ...
                        if (input) input.attr('placeholder', 'встав ключ TMDB');
                    } catch (e) { }
                },
                onChange: function (v) {

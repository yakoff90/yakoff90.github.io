(function () {
    ...
    // ВИПРАВЛЕНО: правильний ключ для отримання URL
    var JACRED_URL = Lampa.Storage.get('jacred_url') || Lampa.Storage.get('jacred.xyz') || 'jacred.xyz'; 
    // ВИПРАВЛЕНО: правильний ключ для API
    var JACRED_API_KEY = Lampa.Storage.get('jacred_api_key') || ''; 
    var PROXY_TIMEOUT = 5000; // Таймаут прокси
    var PROXY_LIST = [
        'https://api.allorigins.win/raw?url=', // змінено на https
        'https://cors.bwa.workers.dev/' // змінено на https
    ];
    ...
    // Функция для тестирования подключения к JacRed
    function testJacredConnection() {
        if (!JACRED_URL) {
            console.error("MAXSM-RATINGS: JacRed URL not set");
            return;
        }
        console.log("MAXSM-RATINGS: Testing connection to", JACRED_URL);
        fetchWithProxy(JACRED_PROTOCOL + JACRED_URL + '/api/v1.0/torrents?search=test&year=2024', 'test', function(err, data) {
            if (err) {
                console.error("MAXSM-RATINGS: Connection test failed", err);
            } else {
                console.log("MAXSM-RATINGS: Connection test successful", data ? "got response" : "empty response");
            }
        });
    }
    ...
    function getBestReleaseFromJacred(normalizedCard, cardId, callback) {
        ...
        // Додано перевірку на API ключ
        if (!JACRED_API_KEY) {
            if (Q_LOGGING) console.log("MAXSM-RATINGS", "card: " + cardId + ", quality: JacRed: API key is not set. Trying without key...");
            // Продовжуємо без ключа, можливо сервер не вимагає ключ
        }
        ...
        function searchJacredApi(searchTitle, searchYear, exactMatch, strategyName, apiCallback) {
            ...
            // ДОДАНО: використання API ключа
            var apiUrl = JACRED_PROTOCOL + JACRED_URL + '/api/v1.0/torrents?search=' +
                encodeURIComponent(searchTitle) +
                '&year=' + searchYear +
                (exactMatch ? '&exact=true' : '') +
                (JACRED_API_KEY ? '&token=' + JACRED_API_KEY : '') + // додано токен якщо є
                '&uid=' + userId;
    ...
    function applyQualityToCard(card, quality, source, qCacheKey) {
        ...
        if (quality && quality !== 'NO') {
            ...
            if (Q_LOGGING) console.log("MAXSM-RATINGS", "Applied quality " + quality + " to card");
        }
    }
    ...
    function checkSettings() {
        console.log("MAXSM-RATINGS-QUALITY", "Current settings:");
        console.log(" - JacRed URL:", JACRED_URL);
        console.log(" - API Key set:", !!JACRED_API_KEY);
        console.log(" - Quality in lists:", localStorage.getItem('maxsm_ratings_quality_inlist'));
        console.log(" - Show on TV:", localStorage.getItem('maxsm_ratings_quality_tv'));
        console.log(" - Quality enabled:", localStorage.getItem('maxsm_ratings_quality'));
    }
    ...
    function startPlugin() {
        ...
        if (localStorage.getItem('maxsm_ratings_quality_inlist') === 'true') {
            ...
            if (existingCards.length) {
                console.log('MAXSM-RATINGS: Found ' + existingCards.length + ' existing cards');

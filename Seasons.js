(function () {
    ...
    // === ФУНКЦИЯ ОПРЕДЕЛЕНИЯ ЯЗЫКА ПРИЛОЖЕНИЯ ===
    ...
    style.textContent = `
    /* Стиль для мітки сезонів (лівий нижній кут) */
    ...
    /* Стиль для мітки прогресу сезонів (лівий нижній кут) */
    ...
    /* === СТИЛІ ДЛЯ МІТКИ СТАТУСУ СЕРІАЛУ (правий нижній кут) === */
    .card--series-status {
        position: absolute;
        right: 8px;
        bottom: 8px;
    ...
    function createSeasonBadge(content, isComplete, loading) {
    ...
    function addSeasonBadge(cardEl) {
        ...
        var oldBadges = view.querySelectorAll('.card--season-complete, .card--season-progress, .card--series-status');
        ...
        // Тільки для серіалів додаємо мітки
        if (mediaType === 'tv') {
            var loadingBadge = createSeasonBadge('...', false, true);
            ...
            fetchSeriesData(data.id)
                .then(function(tmdbData) {
                    ...
                    if (progressInfo) {
                        ...
                        var seasonBadge = createSeasonBadge(content, isComplete, false);
                        view.appendChild(seasonBadge);

                        setTimeout(function() {
                            seasonBadge.classList.add('show');
                        }, 50);

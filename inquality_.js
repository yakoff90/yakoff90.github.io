    ...
    function InQualityService() {
        ...
        async function translateToUkrainian(items) {
            ...
            for (let item of items) {
                ...
                let translatedTitle = await getUkrainianTitle(title, item.id);
                translated.push({
                    id: item.id,
                    ...
                    original_title: translatedTitle || item.original_title || title,
        ...
        async function getUkrainianTitle(title, id) {
            ...
            return new Promise(resolve => {
                Lampa.TMDB.search(title, function(found) {
                    if (found && found.results && found.results.length) {
                        ...
                        cache[id] = match.title || match.name;
                        resolve(match.title || match.name);
                    } else {
                        resolve(title);
                    }
                }, function() {

!function() {
    ...
    var TMDB_API_KEY = "27489d4d8c9dbd0f2b3e89f68821de34"; // Твій API ключ
    ...
    function InQualityService() {
        ...
        self.list = function(params, onComplete, onError) {
            ...
            network.silent(JSON_URL, async function(json) {
                if (json && json.results && Array.isArray(json.results)) {
                    var items = await translateToUkrainian(json.results);
                    var PAGE_SIZE = 20;
        ...
        async function translateToUkrainian(items) {
            const translated = [];
            for (let item of items) {
                let title = item.title || '';
                let translatedData = await getTMDBData(title, item.id);
                translated.push({
                    id: item.id,
                    poster_path: translatedData.poster_path || item.poster_path || '',
                    vote_average: translatedData.vote_average || item.vote_average || 0,
                    original_language: translatedData.original_language || item.original_language || '',
                    title: translatedData.title || title,
                    original_title: translatedData.title || item.original_title || title,
                    release_date: translatedData.release_date || item.release_date || '',
        ...
        async function getTMDBData(title, id) {
                ...
                Lampa.TMDB.search(title, function(found) {
                    if (found && found.results && found.results.length) {
                        ...
                        cache[id] = {
                            ...
                            poster_path: match.poster_path ? "https://image.tmdb.org/t/p/w500" + match.poster_path : '',
                    ...
                    resolve({});
                }, { language: 'uk-UA', api_key: TMDB_API_KEY, type: 'movie' });
            });
        }
